...
function emailShell(title: string, subtitle: string, body: string) {
  return `
    ...
    <body style="margin:0;padding:0;background:#F3ECE3;font-family:Inter,system-ui,sans-serif;">
      <table width="100%" cellpadding="0" cellspacing="0" style="background:#F3ECE3;padding:40px 20px;">
        <tr><td align="center">
          <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.06);">
            <tr>
              <td style="background:linear-gradient(135deg,#7B4F2A,#5B3516);padding:32px 40px;text-align:center;">
            ...
            <tr><td style="background:#EDE5D9;padding:20px 40px;text-align:center;">
              <p style="color:#7B4F2A;font-size:12px;margin:0;">© ${new Date().getFullYear()} Family Dentistry. All rights reserved.</p>
            </td></tr>
          </table>
        </td></tr>
      </table>
...
function detailsTable(data: { service: string; doctor: string; date: string; time: string }) {
  return `
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#F3ECE3;border-radius:12px;padding:24px;margin:0 0 24px;">
      <tr><td style="padding:8px 0;color:#64748B;font-size:13px;">Service</td><td style="padding:8px 0;color:#3B2210;font-size:14px;font-weight:600;text-align:right;">${data.service}</td></tr>
      <tr><td style="padding:8px 0;color:#64748B;font-size:13px;">Doctor</td><td style="padding:8px 0;color:#3B2210;font-size:14px;font-weight:600;text-align:right;">${data.doctor || 'No preference'}</td></tr>
      <tr><td style="padding:8px 0;color:#64748B;font-size:13px;">Date</td><td style="padding:8px 0;color:#3B2210;font-size:14px;font-weight:600;text-align:right;">${data.date}</td></tr>
...
export async function sendBookingReceivedToClinic(data: EmailData, adminEmails: string[] = []) {
  const html = `
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background:#F3ECE3;border-radius:12px;padding:24px;margin:0 0 24px;">
      <tr><td style="padding:8px 0;color:#64748B;font-size:13px;">Patient</td><td style="padding:8px 0;color:#3B2210;font-size:14px;font-weight:600;text-align:right;">${data.name}</td></tr>
      <tr><td style="padding:8px 0;color:#64748B;font-size:13px;">Email</td><td style="padding:8px 0;color:#3B2210;font-size:14px;font-weight:600;text-align:right;">${data.email}</td></tr>
      <tr><td style="padding:8px 0;color:#64748B;font-size:13px;">Phone</td><td style="padding:8px 0;color:#3B2210;font-size:14px;font-weight:600;text-align:right;">${data.phone || '—'}</td></tr>
      <tr><td style="padding:8px 0;color:#64748B;font-size:13px;">Service</td><td style="padding:8px 0;color:#3B2210;font-size:14px;font-weight:600;text-align:right;">${data.service}</td></tr>
      <tr><td style="padding:8px 0;color:#64748B;font-size:13px;">Doctor</td><td style="padding:8px 0;color:#3B2210;font-size:14px;font-weight:600;text-align:right;">${data.doctor || 'No preference'}</td></tr>
      <tr><td style="padding:8px 0;color:#64748B;font-size:13px;">Date</td><td style="padding:8px 0;color:#3B2210;font-size:14px;font-weight:600;text-align:right;">${data.date}</td></tr>
      <tr><td style="padding:8px 0;color:#64748B;font-size:13px;">Time</td><td style="padding:8px 0;color:#3B2210;font-size:14px;font-weight:600;text-align:right;">${data.time}</td></tr>
...
export async function sendConfirmedToPatient(data: EmailData) {
  const html = `
    <p style="color:#3B2210;font-size:16px;margin:0 0 8px;">Hi <strong>${data.name}</strong>,</p>
    <p style="color:#64748B;font-size:14px;line-height:1.7;margin:0 0 24px;">
      Great news! Your appointment has been <strong style="color:#7B4F2A;">approved</strong>. Here are the details:
    </p>
    ${detailsTable(data)}
  ...
  `;
  return transporter.sendMail({
    from: `"Family Dentistry" <${process.env.EMAIL_USER}>`,
    to: data.email,
    subject: `Appointment Confirmed — ${data.date} at ${data.time}`,
    html: emailShell('Appointment Confirmed!', 'Family Dentistry — Your Healthiest Smile Starts Here', html),
  });
}
...
export async function sendCancelledToPatient(data: EmailData) {
  ...
  return transporter.sendMail({
    ...
    html: emailShell('Appointment Cancelled', 'Family Dentistry — Your Healthiest Smile Starts Here', html),
  });
}
...
export async function sendCompletedToPatient(data: EmailData) {
  const html = `
    <p style="color:#3B2210;font-size:16px;margin:0 0 8px;">Hi <strong>${data.name}</strong>,</p>
    <p style="color:#64748B;font-size:14px;line-height:1.7;margin:0 0 24px;">
      Thank you for visiting Family Dentistry! We hope you had a great experience.
    </p>
    ${detailsTable(data)}
    <p style="color:#64748B;font-size:13px;line-height:1.7;margin:0 0 16px;">
  ...
  return transporter.sendMail({
    from: `"Family Dentistry" <${process.env.EMAIL_USER}>`,
    to: data.email,
    subject: `Thank You for Visiting Family Dentistry!`,
    html: emailShell('Thank You!', 'Family Dentistry — Your Healthiest Smile Starts Here', html),
  });
}
